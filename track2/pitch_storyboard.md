...
- **On-screen:** EPCR 0.950 compound-het call; only BUB1B in the MVA panel carries two rare functional alleles; N1002K AlphaMissense 0.9229 (likely pathogenic); ESM-1v ensemble LLR −0.110 (mild-by-LLR, not L1012P-class; LLR is a relative-stability score, not a functional-neutrality proof).
...
- **VO:** "We take a two-pronged upstream approach. First, direct rescue of the UGA stop allele — but the p.Leu737Ter transcript is almost certainly destroyed by nonsense-mediated decay before any readthrough drug can act, so the mechanistically matched approved option is amlexanox, which both promotes PTC readthrough and inhibits NMD (approved as a topical oral paste; systemic use would need reformulation and patient-cell validation). Ataluren is a readthrough drug with no NMD activity and is only conditionally authorised in the UK/MHRA and is not FDA or EMA approved; other listed non-US jurisdictions are not individually verified here. ELX-02 is a next-generation readthrough candidate, yet it is still investigational and sits in a future-research tier. Second, NMN and NR act on the SIRT2/BUBR1 K668 stabilisation axis, but they are dietary supplements, not approved medications, and are retained only as mechanistic comparators. For organellar damage we lead with approved medications: Ravicti, an FDA/EMA-approved sodium-free 4-PBA prodrug for urea-cycle disorders — the 4-PBA chaperone evidence is from trisomy iPSC neurons, not MVA, so this is an extrapolated test; arimoclomol, FDA-approved in combination with miglustat for Niemann-Pick C; and metformin or rilmenidine as approved mTOR-independent autophagy inducers. p.Asn1002Lys is not assumed to be equivalent to the classic L1012P missense. Rapamycin only rescued fly neuroblast counts, not brain size. Antioxidants and Nrf2 activators are flagged because they can promote cancer progression in an MVA1 child."
...
- **VO:** "We ran a target-first ChEMBL screen across the axis. Adding FKBP1A fixed the rapalog retrieval: sirolimus and everolimus bind FKBP12, not the MTOR kinase domain. The approved-only filter gives 32 target-screen rows — these are mechanistically eligible drug-target matches, not final therapeutic leads. We then manually added Ataluren, ELX-02 (investigational), Ravicti, trehalose and spermidine (supplements/food), and dasatinib with quercetin as a conditional senolytic adjunct, then filtered each by regulatory status. In L2S2, each drug has a non-directional p-value, a mimic p-value, and a reverse p-value. The firewall gates on reverse FDR. The result: ACCEPT=0, WEAK=2, REJECT=674 — no drug has a clean, consistent reverse-only L2S2 signal. Sirolimus produced one FDR-significant reverse signature in the pooled human MVA contrast, but it is a mimic in several other contexts, so it is an exploratory, context-specific signal, not a cross-species validation. A rescue-sorted re-query with `sortby=adj_pvalue_down` reproduced the same zero-ACCEPT conclusion. Everolimus had a mimic signature but no significant reverse and is not promoted. Dasatinib has a stronger single reverse p-value but is downgraded to a conditional, oncology-supervised senolytic. Perhexiline was rejected for pediatric toxicity."
...
- **Visual:** validation flowchart: (1) NMD-rescue (RT-qPCR) then Western blot for ≥10% full-length BUBR1 after amlexanox ± ataluren (ELX-02 future), (2) SIRT2/BUBR1 stabilisation assay with NMN/NR as mechanistic comparators, (3) aggregate/apoptosis after approved proteostasis drugs (Ravicti/arimoclomol), (4) TFEB/autophagy after approved inducers (metformin/rilmenidine) or supplement comparators (trehalose/spermidine), (5) ISG/IL-6 biomarker before JAK/IL-6 blockade, (6) oncology surveillance throughout.
...
- [ ] State on-screen or in VO: "Preclinical N-of-1 research proposal, not clinical advice or a treatment recommendation."
